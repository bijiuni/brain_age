...
def loadData(sub_ID):
    ...
    try:
        training_img = nib.load(INPUT_FOLDER + "\\" + str(sub_ID) + "\\PROCESSED\\MPRAGE\\SUBJ_111\\" + str(sub_ID) + "_mpr_n4_anon_sbj_111.img")
    except:
        training_img = nib.load(INPUT_FOLDER + "\\" + str(sub_ID) + "\\PROCESSED\\MPRAGE\\SUBJ_111\\" + str(sub_ID) + "_mpr_n3_anon_sbj_111.img")
...
for index, row in selected_rows.iterrows():
    ...
    print(str(osingle_data.shape) + "  --->   " + str(single_data.shape) + "  --->   " + str(single_resampled.shape) + "  --->   " + str(single_padded.shape))
